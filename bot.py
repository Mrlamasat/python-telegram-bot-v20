...
def encrypt_text(text):
    # نقوم بإضافة "فاصل غير مرئي" بين كل حرفين لكسر التعرف الآلي
    return "‌".join(list(text))

app = Client("mo_secure_bot", api_id=API_ID, api_hash=API_HASH, bot_token=BOT_TOKEN, workers=20)
...
async def on_video(client, message):
    ...
    await message.reply_text("✅ استلمت الفيديو.. أرسل البوستر الآن واكتب (العنوان) في وصف الصورة")
...
async def on_poster(client, message):
    ...
    file_uid = message.photo.file_unique_id if message.photo else message.document.file_unique_id
    
    # تشفير العنوان المرسل في وصف الصورة
    raw_title = message.caption or "مسلسل جديد"
    secure_title = encrypt_text(raw_title)

    db_query("UPDATE temp_upload SET poster_id=%s, poster_uid=%s, title=%s, step='awaiting_ep' WHERE chat_id=%s", 
             (file_id, file_uid, secure_title, message.chat.id), commit=True)
...
async def set_quality_and_publish(client, query):
    ...
    db_query("""INSERT INTO episodes (v_id, poster_id, poster_uid, title, ep_num, duration, quality) 
                ...
                SET poster_id=EXCLUDED.poster_id, title=EXCLUDED.title, quality=EXCLUDED.quality""", 
...
async def start(client, message):
    ...
    # الربط التلقائي عبر الهوية الفريدة للصورة (poster_uid)
