...
app = Client("mo_pro_vFinal_Fixed", api_id=API_ID, api_hash=API_HASH, bot_token=BOT_TOKEN, workers=20)
...
def init_db():
    # إنشاء الجداول وتحديث الأعمدة تلقائياً
...
async def start(client, message):
    ...
    if data:
        ...
        for ep in related_eps:
            ...
            row.append(InlineKeyboardButton(text, url=f"https://t.me/{bot_info.username}?start={ep['v_id']}"))
            if len(row) == 5: buttons.append(row); row = []
        if row: buttons.append(row)

        cap = f"🎬 **{data['title']} - حلقة {data['ep_num']}**\n⚙️ الجودة: {data['quality']}\n\n📌 **باقي الحلقات:**"
        try:
            # محاولة نسخ الرسالة من قناة الأدمن
            ...
            # تنبيه محمد بالخطأ الحقيقي
            await message.reply_text(f"❌ عذراً، لم أستطع سحب الفيديو.\nالسبب التقني: {e}\nتأكد أن البوت مسؤول في قناة الأدمن.")
    else:
