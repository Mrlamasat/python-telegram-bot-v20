...
def extract_link_data(link: str):
    """استخرج channel و message_id من رابط تيليجرام"""
    try:
...
# ==============================
# 🔄 جلب الحلقات القديمة تلقائيًا
# ==============================
async def fetch_old_videos():
    async for message in app.iter_history(SOURCE_CHANNEL, limit=200):
        if message.video:
            v_id = str(message.id)
            title = message.caption or f"فيديو {v_id}"
            poster_id = message.video.thumbs[0].file_id if message.video.thumbs else None
            channel_username = SOURCE_CHANNEL.replace("@", "")
            post_link = f"https://t.me/{channel_username}/{v_id}"

            await db_execute(
                """
                INSERT INTO episodes (v_id, title, poster_id, post_link)
                VALUES ($1, $2, $3, $4)
                ON CONFLICT (v_id)
                DO UPDATE SET title=EXCLUDED.title, poster_id=EXCLUDED.poster_id, post_link=EXCLUDED.post_link
                """,
                v_id, title, poster_id, post_link
            )
    print("✅ تم جلب كل الحلقات القديمة.")

# ==============================
# 🔄 جلب الحلقات الجديدة تلقائيًا عند الرفع
# ==============================
...
async def handle_source_video(client, message):
    ...
    await message.reply_text(f"✅ تم إضافة الحلقة الجديدة: {title}")
...
async def search_or_link(client, message):
    txt = message.text.strip()
    if txt.startswith("https://t.me/"):
        # تشغيل من الرابط
        channel_username, message_id = extract_link_data(txt)
        if not channel_username or not message_id:
            return await message.reply_text("❌ الرابط غير صالح.")
        data = await db_execute("SELECT * FROM episodes WHERE v_id=$1", str(message_id), fetchrow=True)
        if not data:
        ...
        try:
            await client.copy_message(
                ...
                caption=f"**{hide_text(data['title'])}**"
            )
        except Exception as e:
...
async def start(client, message):
    ...
    try:
        ...
        await message.reply_text(f"⚠️ حدث خطأ أثناء جلب الحلقة: {e}")
