...
async def import_updated_series(client, message):
    ...
    async for msg in client.get_chat_history(ADMIN_CHANNEL):
        ...
        # نستخدم الوصف الحالي للبوستر فقط
        caption = (msg.caption or "").strip()
        if not caption:
            continue  # نتجاهل الفيديوهات بدون وصف

        title = caption.lower()  # اسم المسلسل
        ep_num = None
        quality = "غير محدد"

        # حاول استخراج رقم الحلقة من الكابشن
        ...
        if not ep_num:
            ep_num = "1"  # لو لم يوجد رقم، نفترض 1 مؤقتًا
