...
app = Client("mo_pro_final_v6", api_id=API_ID, api_hash=API_HASH, bot_token=BOT_TOKEN, workers=20)
...
def init_db():
    ...
    # 2. إصلاح مشكلة العمود الناقص (تحديث تلقائي)
    try:
        ...
        logger.info(f"ℹ️ Database column update info: {e}")
...
async def on_edit_poster(client, message):
    ...
    if f_uid:
        ...
        await message.reply_text(f"✅ تم تحديث وتشفير العنوان لكل الحلقات المرتبطة.")
