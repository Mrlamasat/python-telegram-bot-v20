...
SOURCE_CHANNEL = -1003547072209  # قناة المصدر (الملفات)
...
# --- 1. التعامل مع أوامر التشغيل (Start) وروابط الحلقات ---
@app.on_message(filters.command("start") & filters.private)
async def start_handler(client, message):
    if len(message.command) > 1:
        # إذا دخل المستخدم عبر رابط حلقة مثل: ?start=3514
        v_id = message.command[1]
        try:
            # نقوم بإرسال الفيديو مباشرة من قناة المصدر للمستخدم
            ...
            await message.reply_text("⚠️ المعذرة، يبدو أن هذا الفيديو غير موجود حالياً.")
    else:
        # إذا أرسل المستخدم /start فقط، نعرض له قائمة المسلسلات
        await send_main_menu(message)

# --- 2. دالة إرسال القائمة الرئيسية ---
async def send_main_menu(message):
    try:
        ...
        if not titles:
            return await message.reply_text("📭 القائمة فارغة حالياً.")

        keyboard = []
        for title in titles:
            # callback_data لا تتحمل أكثر من 64 حرف، لذا نستخدم أول 20 حرف
...
async def show_episodes(client, callback_query):
    try:
        ...
        # البحث عن الحلقات التي يبدأ اسمها بهذا النص
        cur.execute("SELECT ep_num, v_id FROM videos WHERE title LIKE %s ORDER BY ep_num ASC;", (short_name + '%',))
        episodes = cur.fetchall()
        
        if not episodes:
            return await callback_query.answer("⚠️ لا توجد حلقات حالياً.", show_alert=True)

        keyboard = []
        row = []
        for ep_num, v_id in episodes:
            # زر الحلقة يوجه المستخدم لفتح البوت من جديد مع كود الفيديو
            row.append(InlineKeyboardButton(f"حلقة {ep_num}", url=f"https://t.me/{(await client.get_me()).username}?start={v_id}"))
            if len(row) == 3:
...
print("🚀 البوت يعمل الآن بكامل طاقته يا محمد...")
app.run()
