...
def db_query(query, params=(), fetch=True):
    ...
    try:
        pool = get_pool()
        conn = pool.getconn()
        conn.set_client_encoding('UTF8')  # Ø¯Ø¹Ù… Ø§Ù„Ø¹Ø±Ø¨ÙŠØ©
        cur = conn.cursor()
...
def init_db():
    ...
    # Ø¥Ø¶Ø§ÙØ© Ø§Ù„Ø£Ø¹Ù…Ø¯Ø© Ø§Ù„Ù†Ø§Ù‚ØµØ© Ø¨Ø£Ù…Ø§Ù† (Ù„Ø§ ØªØ­Ø°Ù Ø§Ù„Ø¨ÙŠØ§Ù†Ø§Øª Ø§Ù„Ù…ÙˆØ¬ÙˆØ¯Ø©)
    missing_columns = [
        ("ep_num",    "INTEGER"),
        ("poster_id", "TEXT"),
        ("quality",   "TEXT"),
    ...
    # Ø¶Ù…Ø§Ù† Ø¯Ø¹Ù… UTF-8 / Ø§Ù„Ø¹Ø±Ø¨ÙŠØ© ÙÙŠ Ù‚Ø§Ø¹Ø¯Ø© Ø§Ù„Ø¨ÙŠØ§Ù†Ø§Øª
