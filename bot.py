...
app = Client("CinemaBot_V3", api_id=API_ID, api_hash=API_HASH, bot_token=BOT_TOKEN)
...
async def send_episode_details(client, chat_id, v_id):
    ...
    if not ep:
        return await client.send_message(chat_id, "❌ عذراً، لم يتم العثور على الحلقة في قاعدة البيانات.")
...
async def on_start(client, message):
    if len(message.command) > 1:
        ...
        await message.reply_text("أهلاً بك يا محمد. أرسل فيديو في قناة الإشراف للبدء.")
...
async def on_quality(client, query):
    ...
    # --- المعالجة الآمنة للبوستر ---
    try:
        file_path = await asyncio.wait_for(client.download_media(poster_id), timeout=15)
