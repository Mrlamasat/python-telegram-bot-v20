...
app = Client("mo_pro_vFinal_LegacyFixed", api_id=API_ID, api_hash=API_HASH, bot_token=BOT_TOKEN, workers=20)
...
async def on_poster(client, message):
    ...
    await message.reply_text(f"🔢 أرسل رقم الحلقة للعنوان: {secure_title}")
...
# ==============================
# نظام التشغيل (دعم الحلقات القديمة والجديدة)
# ==============================
...
async def start(client, message):
    ...
    if data:
        buttons = []
        # محاولة بناء الأزرار فقط إذا كانت الحلقة جديدة (تملك poster_uid)
        if data.get('poster_uid'):
            ...
            for ep in related:
                ...
                ep_url = f"https://t.me/{bot_info.username}?start={clean_id}"
        ...
        try:
            ...
            await message.reply_text(f"❌ عذراً محمد، حدث خطأ في النسخ: {e}")
    else:
        await message.reply_text("❌ هذه الحلقة غير مسجلة في النظام الجديد.")
