...
# 1. إعدادات التسجيل لمراقبة الأداء في Railway
...
# --- 3. إدارة قاعدة البيانات (حل مشكلة الاتصال نهائياً) ---
def db_query(query, params=(), fetchone=False, fetchall=False, commit=False):
    conn = None
    res = None
    try:
        # تصحيح الرابط وفرض اتصال PostgreSQL عبر الشبكة
        url = DATABASE_URL
        if url and url.startswith("postgres://"):
            url = url.replace("postgres://", "postgresql://", 1)
        
        # الاتصال مع تفعيل SSL لمنع خطأ Socket
...
# --- 4. دالة عرض الحلقة (مع التسجيل التلقائي) ---
async def send_episode_details(client, chat_id, v_id):
    ...
    # ميزة التسجيل التلقائي للروابط القديمة
    if not ep:
        try:
            ...
            if msg and (msg.video or msg.document):
                ...
                # حفظها فوراً لتعمل مستقبلاً
                db_query("INSERT INTO episodes (v_id, poster_id, title, ep_num, duration, quality) VALUES (%s, %s, %s, %s, %s, %s) ON CONFLICT DO NOTHING", 
                         (v_id, "default", title, 0, duration, "Auto"), commit=True)
                ep = ("default", title, 0, duration, "Auto")
            else:
                return await client.send_message(chat_id, "❌ عذراً، المعرف غير موجود أو الفيديو محذوف.")
        except Exception as e:
    ...
    try:
        ...
        # قائمة الحلقات المتسلسلة (فقط إذا لم يكن التسجيل تلقائياً)
        buttons = []
...
async def on_video(client, message):
    ...
    await message.reply_text("✅ استلمت الفيديو. أرسل البوستر (صورة/ملصق):")
...
async def on_text(client, message):
    ...
    await message.reply_text("✨ اختر الجودة للنشر:", reply_markup=InlineKeyboardMarkup([[
...
async def on_quality(client, query):
    ...
    db_query('''INSERT INTO episodes (v_id, poster_id, title, ep_num, duration, quality) 
                VALUES (%s, %s, %s, %s, %s, %s) ON CONFLICT (v_id) 
                DO UPDATE SET poster_id=EXCLUDED.poster_id, ep_num=EXCLUDED.ep_num, quality=EXCLUDED.quality''', 
...
async def on_start(client, message):
    if len(message.command) > 1:
        ...
        await message.reply_text(f"أهلاً بك يا محمد! البوت جاهز للعمل مع قاعدة بيانات PostgreSQL.")
