...
# --- المعرفات المعدلة للاختبار يا محمد ---
ADMIN_CHANNEL = -1003547072209  # قناة التخزين (الخاصة)
TEST_CHANNEL = "@khofkrjrnrqnrnta"  # قناة الاختبار الجديدة
REQ_CHANNEL = "RamadanSeries26" # قناة الاشتراك (لأغراض الفحص)

app = Client("CinemaBot_Test", api_id=API_ID, api_hash=API_HASH, bot_token=BOT_TOKEN)
...
# --- 2. استلام البوستر (يدعم كافة الصيغ) ---
...
# --- 4. النشر النهائي الذكي (إلى قناة الاختبار) ---
...
async def send_episode_details(client, chat_id, v_id):
    ...
    try:
        ...
        await client.send_message(chat_id, "❌ خطأ في جلب الفيديو.")
...
async def on_start(client, message):
    if len(message.command) > 1:
        ...
        await message.reply_text("أهلاً بك يا محمد في نسخة الاختبار!")
