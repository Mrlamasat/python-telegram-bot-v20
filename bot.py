...
async def send_episode_details(client, chat_id, v_id):
    ...
    try:
        ...
        # إنشاء قائمة أزرار لجميع الحلقات بنفس البوستر
        ...
        logger.error(f"Error sending episode details: {e}")
...
async def on_poster(client, message):
    ...
    if not res or len(res) == 0 or res[0] != "awaiting_poster":
        return
    ...
    db_query("UPDATE temp_upload SET poster_id=%s, title=%s, step=%s WHERE chat_id=%s",
             (poster_id, message.caption or "", "awaiting_ep_num", ADMIN_CHANNEL), commit=True)
...
async def on_text(client, message):
    res = db_query("SELECT step FROM temp_upload WHERE chat_id=%s", (ADMIN_CHANNEL,), fetchone=True)
    if not res or len(res) == 0 or res[0] != "awaiting_ep_num": return
...
async def on_quality(client, query):
    ...
    # محاولة تحويل WebP أو Sticker للعرض
    try:
        ...
        with Image.open(file) as img:
            ...
            bio = io.BytesIO(); bio.name = "poster.png"; bg.save(bio, "PNG"); bio.seek(0)
