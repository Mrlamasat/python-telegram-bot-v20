...
# --- 4. دالة عرض الحلقة (مع إخفاء بيانات التلقائي) ---
async def send_episode_details(client, chat_id, v_id):
    ...
    try:
        ...
        # إذا كانت الحلقة تلقائية (رقمها 0)، نكتفي بإرسال الفيديو فقط
        if ep_num == 0:
            return 

        # الحلقات المرفوعة يدوياً: إرسال التفاصيل والأزرار
...
# --- 5. نظام الرفع الاحترافي (حل مشكلة التوقف) ---
...
async def on_poster(client, message):
    ...
    # تحديث سريع لقاعدة البيانات دون معالجة الصورة الآن لمنع التوقف
...
async def on_quality(client, query):
    ...
    # معالجة البوستر في الخطوة الأخيرة (أكثر استقراراً)
    file_path = None
    try:
...
async def on_start(client, message):
    if len(message.command) > 1:
        ...
        await message.reply_text(f"أهلاً بك يا {message.from_user.first_name}! البوت يعمل بكامل طاقته.")
