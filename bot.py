...
# Session String لحساب المستخدم (Userbot)
SESSION_STRING = os.environ.get("USER_SESSION")
if not SESSION_STRING:
    raise ValueError("❌ USER_SESSION فارغ! ضعها في Variables")
...
# حساب المستخدم لاستيراد الحلقات القديمة
app_user = Client(
    "user_session",
    api_id=API_ID,
    api_hash=API_HASH,
...
async def start(client, message):
    ...
    if data:
        ...
        try:
            ...
            await message.reply_text("⚠️ خطأ في جلب الفيديو، تأكد من إضافة البوت كآدمن.")

# ==============================
# 6. استيراد الحلقات القديمة (/import_updated) عبر حساب المستخدم
# ==============================
@app.on_message(filters.command("import_updated") & filters.private)
async def import_updated_series(client, message):
    await message.reply_text("🔄 بدء الاستيراد من القناة باستخدام Userbot...")
    count = 0
    async with app_user:
        async for msg in app_user.get_chat_history(ADMIN_CHANNEL):
            ...
            db_query("""INSERT INTO episodes (v_id, series_id, ep_num, duration, quality)
                        VALUES (%s, %s, %s, %s, %s)
                        ON CONFLICT (v_id) DO UPDATE
                        SET series_id=EXCLUDED.series_id,
                            ep_num=EXCLUDED.ep_num,
                            quality=EXCLUDED.quality""",
                     (str(msg.id), series_id, int(ep_num), "0:00", quality), commit=True)
            count += 1
