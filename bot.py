...
# إعدادات القنوات التي حددتها يا محمد
...
async def get_episodes_markup(title, current_v_id):
    ...
    # إضافة زر المشاركة الاحترافي
    share_text = urllib.parse.quote(f"🍿 شاهد مسلسل: {title}\n🎬 متوفر الآن بجودة عالية عبر البوت الرسمي 👇")
    share_link = f"https://t.me/{bot_info.username}?start={current_v_id}"
    full_share_url = f"https://t.me/share/url?url={share_link}&text={share_text}"
    buttons.append([InlineKeyboardButton("📢 مشاركة الحلقة مع أصدقائك", url=full_share_url)])
...
# ===== المزامنة: تحديث البيانات عند تعديل وصف الفيديو القديم =====
...
async def sync_old_videos(client, message):
    ...
    # محاولة استخراج رقم الحلقة تلقائياً من الوصف
    ep_num = 1
    nums = re.findall(r'\d+', message.caption or "")
    if nums: ep_num = int(nums[0])
    
    # تحديث قاعدة البيانات وربط الفيديو بالعنوان
    db_query("""
