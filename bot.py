...
async def on_video(client, message):
    ...
    db_query("""
        INSERT OR REPLACE INTO temp_upload (chat_id, v_id, duration, step)
        VALUES (?, ?, ?, ?)
    """, (ADMIN_CHANNEL, v_id, duration, "awaiting_poster"), commit=True)

    await message.reply_text("✅ تم استلام الفيديو\n🖼 الآن أرسل البوستر:")

# --- 2. استلام البوستر (photo أو document) ---
@app.on_message(filters.chat(ADMIN_CHANNEL) & (filters.photo | filters.document))
async def on_poster(client, message):
    res = db_query("SELECT step, v_id FROM temp_upload WHERE chat_id=?", (ADMIN_CHANNEL,), fetchone=True)
    if not res or res[0] != "awaiting_poster":
        return

    try:
        # دعم photo أو document (JPEG, PNG, WEBP)
        if message.photo:
            file_id = message.photo.file_id
        elif message.document and message.document.mime_type.startswith("image/"):
            file_id = message.document.file_id
        else:
            return await message.reply_text("❌ هذا الملف ليس صورة صالحة للبوستر.")

        title = message.caption or ""
        v_id = res[1]

        db_query("""
            INSERT OR REPLACE INTO temp_upload (chat_id, v_id, poster_id, title, step)
            VALUES (?, ?, ?, ?, ?)
        """, (ADMIN_CHANNEL, v_id, file_id, title, "awaiting_ep_num"), commit=True)

        await message.reply_text("🖼 تم حفظ البوستر بنجاح\n🔢 أرسل الآن رقم الحلقة:")
    except Exception as e:
        logging.error(f"Error in poster: {e}")
        await message.reply_text("⚠️ حدث خطأ أثناء حفظ البوستر، حاول إرساله مرة أخرى.")
...
async def on_quality(client, query):
    ...
    caption = ""
    if title.strip(): caption += f"🎬 **{title}**\n"
...
# --- إرسال تفاصيل الحلقة للمستخدم ---
async def send_episode_details(client, chat_id, v_id):
    ep = db_query("SELECT poster_id, title, ep_num, quality, duration FROM episodes WHERE v_id=?", (v_id,), fetchone=True)
    if not ep: 
        await client.send_message(chat_id, "❌ عذراً، لم يتم العثور على هذه الحلقة.")
        return

    poster_id, title, ep_num, quality, duration = ep
