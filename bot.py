...
# ===== [3] عرض الحلقة (النسخة المطورة للاستخراج والتعديل) =====
async def show_episode(client, message, v_id, edit=False):
    ...
    if not res:
        try:
            ...
            raw_text = source_msg.caption if source_msg.caption else "مسلسل قديم"
            
            # استخراج الرقم تلقائياً
            nums = re.findall(r"(\d+)", raw_text)
            ep_num = int(nums[0]) if nums else 0
            
            # تنظيف الاسم من الكلمات المكررة
            title = re.sub(r"الحلقة|حلقة|\d+", "", raw_text).replace(":", "").strip()
            if not title: title = "مسلسل غير مسمى"
...
async def set_episode_data(client, message):
    if message.from_user.id != ADMIN_ID: return
    cmd = message.text.split(maxsplit=3)
    if len(cmd) < 3:
        return await message.reply_text("❌ الصيغة: `/set ID الرقم` أو `/set ID الرقم الاسم`")

    v_id_to_fix, new_ep = cmd[1], cmd[2]
    if len(cmd) == 4:
        db_query("UPDATE videos SET ep_num = %s, title = %s WHERE v_id = %s", (new_ep, cmd[3], v_id_to_fix), fetch=False)
        await message.reply_text(f"✅ تم تحديث الحلقة {v_id_to_fix} بنجاح.")
    else:
        db_query("UPDATE videos SET ep_num = %s WHERE v_id = %s", (new_ep, v_id_to_fix), fetch=False)
        await message.reply_text(f"✅ تم تحديث رقم الحلقة {v_id_to_fix} إلى {new_ep}")
...
async def handle_source(client, message):
    if message.video or message.document:
        ...
        if res:
            ...
            await client.send_photo(PUBLIC_POST_CHANNEL, p_id, f"🎬 <b>{title}</b>\n<b>الحلقة: [{message.text}]</b>\n<b>رابط المشاهدة:</b>\n{watch_link}", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("▶️ مشاهدة", url=watch_link)]]))
...
async def start_cmd(client, message):
    if len(message.command) > 1:
        ...
        await message.reply_text("أهلاً بك يا محمد. البوت يعمل وجاهز!")
