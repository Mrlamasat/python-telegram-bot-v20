...
# --- 2. استلام البوستر (هنا التعديل الجذري) ---
...
async def on_poster(client, message):
    ...
    # جلب الوصف إن وجد، أو وضع كلمة 'none' كعلامة برمجية فقط
    caption = message.caption if message.caption else "none"
    photo_id = message.photo.file_id
    
    # تحديث البيانات والانتقال للخطوة التالية
    db_query("UPDATE temp_upload SET poster_id=?, title=?, step=? WHERE chat_id=?", 
             (photo_id, caption, "awaiting_ep_num", ADMIN_CHANNEL), commit=True)
...
async def on_quality(client, query):
    ...
    # بناء الرسالة: إذا كان العنوان 'none' لا يظهره
    caption = ""
