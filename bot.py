...
ADMIN_CHANNEL = -1003547072209  # ايدي القناة
...
# ==============================
# 3. استيراد الحلقات القديمة (Import)
# ==============================
...
async def import_updated_series(client, message):
    ...
    try:
        ...
        async for msg in client.get_chat_history(target_chat.id):
            ...
            # الاسم: أول سطر في الوصف أو اسم افتراضي
            clean_title = caption.split('\n')[0].replace('🎬','').strip() if caption else "مسلسل غير معروف"
            ...
            # إضافة المسلسل أو الحصول على series_id
...
async def publish(client, query):
    ...
    db_query("INSERT INTO episodes (v_id, series_id, poster_id, title, ep_num, duration, quality) VALUES (%s,%s,%s,%s,%s,%s,%s) "
             "ON CONFLICT (v_id) DO UPDATE SET series_id=EXCLUDED.series_id, ep_num=EXCLUDED.ep_num",
             (data['v_id'], s_data['id'], data['poster_id'], data['title'], data['ep_num'], data['duration'], quality),
             commit=True)
...
async def start(client, message):
    ...
    if data:
        ...
        try:
            ...
            await message.reply_text("❌ خطأ، تأكد من البوت كأدمن في قناة الأرشيف.")
    else:
